...
<<<<<<< HEAD
                                        tasks[task_id].update({
                                            ...
                                            "execution_time": (shape_result.get("execution_time", 0) or 0) + 
                                                            (paint_result.get("execution_time", 0) or 0),
                                        })
                                        tasks[task_id]["status"] = "completed"
                                        print(f"Task {task_id}: Paint generation completed successfully")
                                    else:
                                        tasks[task_id]["status"] = "failed"
                                        tasks[task_id]["error"] = paint_result.get("error", "Paint generation failed")
                                        print(f"Task {task_id}: Paint generation failed: {paint_result.get('error')}")
                                else:
...
<<<<<<< HEAD
                            ...
                            tasks[task_id]["status"] = "failed"
                            tasks[task_id]["error"] = f"Unexpected error during paint generation: {str(e)}"
                            print(f"Task {task_id}: Unexpected error during paint generation: {str(e)}")
                except Exception as e:
                    tasks[task_id]["status"] = "failed"
