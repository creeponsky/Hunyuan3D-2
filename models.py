...
class ModelManager:
    ...
    def initialize(self, gpu_id: int) -> None:
        ...
        try:
            ...
            # Verify GPU setting
            if torch.cuda.current_device() != 0:  # Because of CUDA_VISIBLE_DEVICES, current device should always be 0
                raise RuntimeError(f"Failed to set GPU device. Current device: {torch.cuda.current_device()}")

            self.gpu_id = gpu_id
            print(f"ModelManager initialized on GPU {gpu_id} ({torch.cuda.get_device_name(0)})")
